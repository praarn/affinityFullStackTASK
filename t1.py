...
class MDComputersScraper:
    ...
    def search_page(self, term: str, page: int = 1) -> (List[Product], int):
        soup = self._get(self._search_url(term, page))
    ...
    def search(self, term: str, max_pages: Optional[int] = None) -> List[Product]:
        all_products: List[Product] = []
        page = 1
        total_pages = 1
 
        while True:
            products, total_pages = self.search_page(term, page)
            if not products:
                break
...
def main():
    ...
    if args.details:
        ...
        products = scraper.search(args.term, max_pages=args.pages)
